...
int main (int argc, char const *argv[]){
  ...
  while (1){
	...
	sem_post(&sem_prot_quero);
   	 ...
   	 mi_clk = clk + 1;
	for (int i = 0; i < N-1; i++){
  	sendMsg(REQUEST, id_nodos[i], mi_clk);
	}
	for (int i = 0; i < N-1; i++){
	...
	if(mi_prio == 4) {
  	for (int i = 0; i < num_pend; i++) {
      	sendMsg(REPLY, id_nodos_pend[i], 0);
      	num_pend = 0;
  	}
	}
...
void *fillo (void *args){
  ...
  default : strcpy(proceso, "proceso desconocido"); break;
    ...
    *contador = *contador + 1;
  if(*prio==4){
  n_lectores_cola= n_lectores_cola +1;
  }
    sem_post(sem_contador);

 if(!(*prio==4&&lectura==1)){
  lectura=0;
    sem_post(&sem_entrada);//AQUI PEDIMOSA EXCLUSION MUTUA A NIVEL DISTRIBUIDO
  sem_wait(sem_proceso);
}

  sem_wait(sem_contador);
  n_lectores_cola= n_lectores_cola -1;
  if(n_lectores_cola!=0&&n_anulacions==0&&n_pagos==0&&n_reservas==0){
